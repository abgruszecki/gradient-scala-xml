...
object NodeSeq {
  ...
  def newBuilder(/*GRADIENT*/acc: Acc^): mutable.Builder[Node, NodeSeq]^{acc} = acc.new mutable.ListBuffer[Node].mapResult(fromSeq)
...
abstract class NodeSeq extends AbstractSeq[Node] with immutable.Seq[Node] with ScalaVersionSpecificNodeSeq with Equality with Serializable {
  ...
  override def iterator(/*GRADIENT*/acc: Acc^): Iterator[Node] = theSeq.iterator(acc)
