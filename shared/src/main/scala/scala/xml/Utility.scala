...
object Utility extends AnyRef with parsing.TokenTests {
   ...
   */
  def collectNamespaces(nodes: Seq[Node])(/*GRADIENT*/acc: Acc^): mutable.Set[String] =
    nodes.foldLeft(acc.new mutable.HashSet[String]) { (set, x) => collectNamespaces(x, set); set }
  ...
  def toXML(
    x: Node,
    pscope: NamespaceBinding = TopScope,
    sb: StringBuilder^ = { /*GRADIENT*/ val local = new Module {}; local.new StringBuilder },
    stripComments: Boolean = false,
    decodeEntities: Boolean = true,
    preserveWhitespace: Boolean = false,
