...
trait NodeFactory[A <: Node](/*GRADIENT*/acc: Acc^) {
  val ignoreComments: Boolean = false
  val ignoreProcInstr: Boolean = false

  /* default behaviour is to use hash-consing */
  val cache: scala.collection.mutable.HashMap[Int, List[A]]^{acc} = acc.new scala.collection.mutable.HashMap[Int, List[A]]
