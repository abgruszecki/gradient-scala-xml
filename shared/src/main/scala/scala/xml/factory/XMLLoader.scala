 ...
 */
trait XMLLoader[T <: Node](/*GRADIENT*/acc: Acc^) { self =>
  ...
  private lazy val parserInstance: ThreadLocal[SAXParser^#]^{acc} = acc.new ThreadLocal[SAXParser^#] {
  ...
  def adapter: parsing.FactoryAdapter^# { val acc: self.acc.type } = acc.new parsing.NoBindingFactoryAdapter(acc)

  /** Loads XML Document. */
  def loadDocument(inputSource: InputSource^#): Document^{acc} { val acc: self.acc.type } = adapter.loadDocument(inputSource, reader)
  def loadFileDocument(fileName: String): Document^{acc} { val acc: self.acc.type } = loadDocument(Source.fromFile(fileName))
  def loadFileDocument(file: File^): Document^{acc} { val acc: self.acc.type } = loadDocument(Source.fromFile(file))
  def loadDocument(url: URL): Document^{acc} { val acc: self.acc.type } = loadDocument(Source.fromUrl(url))
  def loadDocument(sysId: String): Document^{acc} { val acc: self.acc.type } = loadDocument(Source.fromSysId(sysId))
  def loadFileDocument(fileDescriptor: FileDescriptor^): Document^{acc} { val acc: self.acc.type } = loadDocument(Source.fromFile(fileDescriptor))
  def loadDocument(inputStream: InputStream^): Document^{acc} { val acc: self.acc.type } = loadDocument(Source.fromInputStream(inputStream))
  def loadDocument(reader: Reader^): Document^{acc} { val acc: self.acc.type } = loadDocument(Source.fromReader(reader))
  def loadStringDocument(string: String): Document^{acc} { val acc: self.acc.type } = loadDocument(Source.fromString(string))
