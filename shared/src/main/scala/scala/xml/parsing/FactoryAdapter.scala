 ...
 */
abstract class FactoryAdapter(/*GRADIENT*/acc: Acc^) extends DefaultHandler2 with factory.XMLLoader[Node](acc) { self =>
  ...
  private var dtdBuilder: Option[DtdBuilder^{acc} { val acc: self.acc.type }] = None
  private def inDtd: Boolean = dtdBuilder.isDefined && !dtdBuilder.get.isDone

  private var document: Option[Document^{acc} { val acc: self.acc.type }] = None
  ...
  val buffer: StringBuilder^{acc} = acc.new StringBuilder()
   ...
   */
  def loadDocument(inputSource: InputSource^#, xmlReader: XMLReader^#): Document^{acc} { val acc: self.acc.type } = {
