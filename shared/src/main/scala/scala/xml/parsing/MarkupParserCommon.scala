...
private[scala] trait MarkupParserCommon(/*GRADIENT*/acc: Acc^) extends TokenTests {
  ...
  def lookahead(): BufferedIterator[Char]^{acc,net,fs}
  ...
  private def peek(lookingFor: String): Boolean =
    lookahead().take(lookingFor.length).sameElements(lookingFor.iterator(acc)) && {
